    ...
    <div className="space-y-4">
      {sections.map((section, index) => {
        switch (section.type) {
          case 'correct':
            return (
              <div key={index} className="flex items-start gap-3 p-4 bg-green-500/10 border border-green-500/30 rounded-xl">
          ...
          case 'wrong_header':
            return (
              <div key={index} className="flex items-center gap-2 mt-6 mb-3">
                ...
                <h4 className="text-red-300 font-medium">{section.content}</h4>
              </div>
            );
          ...
          case 'wrong_item':
            ...
            return (
              <div key={index} className="flex items-start gap-3 p-3 bg-red-500/10 border border-red-500/20 rounded-lg ml-2">
                <div className="w-6 h-6 bg-red-500/20 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                  <span className="text-red-300 text-sm font-medium">{option}</span>
                </div>
